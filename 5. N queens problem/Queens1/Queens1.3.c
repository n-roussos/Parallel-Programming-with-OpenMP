...
int main(){
	...
	int tries = 0;
	int valid_row;
	bool reposition = false;	//no queen is repositioned initially

	for(int col=0;col<N;){
		if(reposition){			//only if a queen changes position
			calc_valid(col);	//the valid rows should be updated
			reposition = false;	//reset flag
		}
		...
		if(valid_row >=0){
			...
			tries++;
			reposition = true;	//queen repositioned
		}else{
			...
			ROWS_PLACED[col--] = -1;	//remove queen & backtrack to previous column
		}
	}
